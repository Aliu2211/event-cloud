...
export function TicketView({ registration: initialRegistration, event }: { registration: Registration; event: EventRecord }) {
  const [registration, setRegistration] = useState(initialRegistration);
  const [cancelling, setCancelling] = useState(false);
  const [error, setError] = useState("");

  async function handleCancel() {
    if (!window.confirm("Cancel this registration? This can't be undone.")) return;
    setError("");
    setCancelling(true);
    try {
          ...
          <div className="print:hidden flex items-center gap-4">
            {registration.status === "confirmed" && (
              <button
                ...
                disabled={cancelling}
                className="flex items-center gap-1 text-xs font-medium text-error hover:underline disabled:opacity-60"
              >
